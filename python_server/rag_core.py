...
class NoteRAG:
    ...
    def _init_index(self):
        ...
        This function:
        - Checks if an existing index is stored on disk for the current user
        ...
        try:
            ...
            if os.path.exists(os.path.join(self.persist_dir, "docstore.json")):
                logger.debug(f"Found existing index for {self.user_email or 'default user'}, loading...")
                # Load existing index
                storage_context = StorageContext.from_defaults(persist_dir=self.persist_dir)
                self.index = load_index_from_storage(storage_context)
                logger.info(f"Successfully loaded existing index for {self.user_email or 'default user'}")
            else:
                logger.debug(f"No existing index found for {self.user_email or 'default user'}, creating new one")
                ...
                logger.info(f"Successfully created and persisted new index for {self.user_email or 'default user'}")
    ...
    def add_note(self, text: str, title: str, timestamp: int) -> Dict:
        ...
        try:
            ...
            # Add user_email to metadata if available
            if self.user_email:
