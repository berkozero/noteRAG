/**
 * Advanced search service for notes
 * Combines ChromaDB vector store for semantic search with keyword search
 */
const vectorStore = require('../vectordb/chroma-store');
...
async function init() {
  ...
  try {
    ...
    const vectorStoreInitialized = await vectorStore.init();
    if (!vectorStoreInitialized) {
      logger.warn('Vector store initialization failed, semantic search may not work properly');
    }
    
    logger.info('Advanced search service initialized successfully');
    return true;
  } catch (error) {
    logger.error('Failed to initialize advanced search service', error);
    return false;
  }
}
...
/**
 * Perform a semantic search on notes using the vector store
 * @param {string} query - The search query
 * @param {Object} options - Search options
 * @param {number} [options.limit=10] - Maximum number of results
 * @param {number} [options.threshold=0.1] - Minimum similarity score
...
async function semanticSearch(query, options = {}) {
  const { 
    ...
    threshold = 0.1, // Much lower threshold for better concept matching
    includeScores = false 
  } = options;
  ...
  try {
    ...
    const results = await vectorStore.queryDocuments(query, {
      limit,
      threshold,
      includeScores
    });
    ...
    logger.debug('Falling back to keyword search due to semantic search error');
...
async function hybridSearch(query, options = {}) {
  const { 
    limit = config.search.maxResultsPerPage, 
    threshold = 0.5, // Lowered threshold for better related concept matching
    includeScores = false 
  } = options;
  ...
  try {
    ...
    // Add semantic results first with higher weight (they're generally more relevant)
    for (const result of semanticResults) {
      combinedResultsMap.set(result.id, {
        ...result,
        source: 'semantic',
        // Semantic scores are often in [0,1] range, scale to ensure they're prioritized
        rank: result.score ? 1 + result.score : 1.5 
      });
    }
    
    // Add keyword results with lower weight if not already included
    for (const result of keywordResults) {
      if (!combinedResultsMap.has(result.id)) {
        combinedResultsMap.set(result.id, {
          ...result,
          source: 'keyword',
          rank: 0.8  // Lower than semantic by default
        });
      }
    }
 ...
 * This is called when a new note is created
 * @param {string} id - Note ID
 * @param {Object} note - Note object
 * @returns {Promise<boolean>} Success status
 */
async function addToIndex(id, note) {
  try {
    // Extract text content from note for embedding
    const content = note.content || '';
    const title = note.title || '';
    const fullText = `${title}\n\n${content}`;
    
    // Add to vector store
    await vectorStore.addDocument(id, fullText, {
      title: note.title,
      url: note.url,
    ...
    logger.debug(`Added note ${id} to search index`);
    return true;
  } catch (error) {
    logger.error(`Error adding note ${id} to search index`, error);
    return false;
  }
}
 ...
 * This is called when a note is updated
 * @param {string} id - Note ID
 * @param {Object} note - Updated note object
 * @returns {Promise<boolean>} Success status
 */
async function updateIndex(id, note) {
  try {
    // Extract text content from note for embedding
    const content = note.content || '';
    const title = note.title || '';
    ...
    await vectorStore.updateDocument(id, fullText, {
      title: note.title,
      url: note.url,
      created: note.created,
      updated: note.updated
    ...
    logger.debug(`Updated note ${id} in search index`);
    return true;
  } catch (error) {
    logger.error(`Error updating note ${id} in search index`, error);
    return false;
  }
}
 ...
 * This is called when a note is deleted
 * @param {string} id - Note ID
 * @returns {Promise<boolean>} Success status
 */
async function removeFromIndex(id) {
  try {
    ...
    logger.debug(`Removed note ${id} from search index`);
    return true;
  } catch (error) {
    logger.error(`Error removing note ${id} from search index`, error);
    return false;
  }
}
